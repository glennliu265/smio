...
ax.set_title("Correlation between SPGNE SST and AMOC Maximum Transport @ each latitude")
...
for ff in range(nfilt):
    ...
    fig,axs  = plt.subplots(2,1,constrained_layout=True,figsize=(12.5,6),sharex=True)
...
def pointwise_coherence(ts1,ts2,opt=1,nsmooth=100,pct=0.10,return_ds=False):
    ...
    if np.any(np.isnan(ts1)) or np.any(np.isnan(ts2)):
        ...
        if return_ds:
            ...
            spectra_x    = xr.DataArray(PX,coords=coords,dims=coords,name="spectra_x")
            spectra_y    = xr.DataArray(PY,coords=coords,dims=coords,name="spectra_y")
            quad_spec    = xr.DataArray(QP,coords=coords,dims=coords,name="quad_spectrum")
            co_spec      = xr.DataArray(CP,coords=coords,dims=coords,name="co_spectrum")
            coherence_sq = xr.DataArray(coherence_sq,coords=coords,dims=coords,name="coherence_sq")
            ds_out = xr.merge([othervars,spectra_x,spectra_y,quad_spec,co_spec,coherence_sq])
            return ds_out
        return coherence_sq
...
for a in tqdm.tqdm(range(nlat)):
    ...
    cs  = pointwise_coherence(ts1,ts2,opt=1,nsmooth=nsmooth,pct=0.10,return_ds=True)
    chsq.append(cs)
...
#%% Now make a plot, starting with a slice


plotvar = chsq2.sel(lat=45,method='nearest')
...
# # Do Montecarlo Simulations (Made Function Version Below, please see that )
...
# arsig2 = []
# coh    = []
# for ii in tqdm.tqdm(range(mciter)):
    
#     ar1 = proc.make_ar1(r1,sigma1,ntime)
#     ar2 = proc.make_ar1(r2,sigma2,ntime)
    
#     arsig1.append(np.std(ar1))
#     arsig2.append(np.std(ar2))
    
#     arcoh = pointwise_coherence(ar1,ar2,opt=1,nsmooth=nsmooth,pct=0.10,return_ds=True)
    
#     coh.append(arcoh.coherence_sq.data)
    
# coh = np.array(coh)

#%%

## Note: Moved function to proc
...
mciter  = 1000
csig    = lambda x,y: calcsig_both(x,y)
ccoh    = lambda x,y: pointwise_coherence(x,y,opt=1,nsmooth=nsmooth,pct=0.10,return_ds=True).coherence_sq.data
...
# testgen     = proc.make_ar1(0.85,sigtest,testlen) #+ np.random.normal(0,1,)

# r1,sigma1   = calc_r1_sigma(testgen)

# sigout = []
# for mc in range(1000):
#     tss = proc.make_ar1(r1,sigma1,testlen)
#     sigout.append(np.std(sigout))

# Testing White Noise
# sigma12 = (1-r1**2)**2
# wn1     = np.random.normal(0,1,ntime)
# wn2     = np.random.normal(0,1,ntime)
# wncoh   = pointwise_coherence(wn1,wn2,opt=1,nsmooth=nsmooth,pct=0.10,return_ds=True)
...
ax.fill_between(plotvar.freq,lowerbnd,upperbnd,alpha=0.45,label="95% Significance")
...
#ax.legend()
if filt is not None:
    ax.set_title("Coherence Squared (%i-month Filtered SPGNE SST vs. AMOC @ %.2f$\degree$N)" % (filt,latf),fontsize=18)
else:
    ax.set_title("Coherence Squared (SPGNE SST vs. AMOC @ %.2f$\degree$N)" % (latf),fontsize=18)
figpath = "/Users/gliu/Downloads/02_Research/01_Projects/05_SMIO/02_Figures/20251006/"
proc.makedir(figpath)

figname = "%sCoherenceSq_SPGNE_AMOC%02i_filt%s_mciter%i.png" % (figpath,latf,filtstr,mciter)
